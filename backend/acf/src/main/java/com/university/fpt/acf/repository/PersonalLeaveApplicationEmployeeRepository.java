...
public interface PersonalLeaveApplicationEmployeeRepository extends JpaRepository<PersonaLeaveApplication,Long> {
    @Query("select new com.university.fpt.acf.vo.SearchPersonalApplicationEmployeeVO(p.id,p.fileAttach,p.title,p.comment,p.content,p.accept) from PersonaLeaveApplication p where p.id=:id")
